...
#Use tail to output the contents of infile with the first line (the header) skipped. Use a pipe to feed the output from tail into sort.
#Use sort to sort the lines of the file. Set the sorting key using -k. 1 indicates the first field (the whole line is considered a single field by sort as there is no space between the comma-separated fields), and 6 indicates that the key begins from the 6th character (this is where the timestamp begins in each line).
...
tail -n +2 $infile | sort -k1.6 >> $outfile
#Note: Should have used a comma delimiter and sorted numerically on field 2